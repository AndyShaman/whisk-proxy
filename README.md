...
- **Токены хранятся только на вашем компьютере** — в папке `~/.whisk-proxy/` (домашняя директория)
...
- **Токены хранятся в домашней директории** — безопасно при глобальной установке

Token location:
- **Mac/Linux:** `~/.whisk-proxy/token.json`
- **Windows:** `%USERPROFILE%\.whisk-proxy\token.json`
...
~/.whisk-proxy/            # Токены (в домашней директории)
└── token.json
```
